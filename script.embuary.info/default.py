...
def perform_warmup():
    """Carrega bibliotecas pesadas na memória para reduzir Cold Start"""
    xbmc.log('[%s] Process Warm-up started' % ADDON_ID, xbmc.LOGINFO)
    try:
        import sqlite3
        import json
        import datetime
        ...
        # Opcional: inicializar cache manager para garantir conexão DB
        _ = get_cache_manager()
        
        xbmc.log('[%s] Process Warm-up finished' % ADDON_ID, xbmc.LOGINFO)
    except Exception as e:
        xbmc.log('[%s] Process Warm-up failed: %s' % (ADDON_ID, str(e)), xbmc.LOGWARNING)

# =============================================================================
# MODO PLUGIN (Para popular listas no XML)
# =============================================================================
def run_as_plugin(handle, params_str):
    try:
        ...
        if mode == 'cast':
            ...
            # Importações tardias (Lazy Loading) economizam memória na inicialização
            from resources.lib.video import TMDBVideos
            ...
            if (not tmdb_id or tmdb_id in ['None', '']) and imdb_id:
                ...
                if cached_tmdb_id:
                    ...
                    try:
                        find_data = tmdb_query(action='find', call=imdb_id, params={'external_source': 'imdb_id'}, show_error=False)
            ...
            for actor in cast_list:
                ...
                if not real_image: real_image = actor.getArt('poster')
                if not real_image: real_image = 'DefaultActorSolid.png'
                ...
                url_action = 'plugin://script.embuary.info/?call=person&query=' + query_name
                
                xbmcplugin.addDirectoryItem(handle=handle, url=url_action, listitem=li, isFolder=False)
            
            plugin_elapsed = time.time() - plugin_start
            xbmc.log('[%s] Plugin CAST completed in %.3fs (%d actors)' % (ADDON_ID, plugin_elapsed, len(cast_list)), xbmc.LOGINFO)
            xbmcplugin.endOfDirectory(handle)
            return
...
class Main:
    def __init__(self):
        ...
        if self.params.get('mode') == 'reset_scroll':
            # Reset Actor Panel Scroll Logic
        ...
        # Dialog só é criado se entrarmos no modo Script real
